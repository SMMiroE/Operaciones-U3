...
def calculate_Y_from_wet_bulb(t_dry_bulb, t_wet_bulb, total_pressure_atm, units_system, psych_const):
    ...
    if Pv < 0: Pv = 0.0
    return 0.62198 * (Pv / (P_total - Pv)) if (P_total - Pv) > 0 else float('inf')
...
try:
    ...
    st.success(f"✅ Pinch calculado a {t_pinch_global:.2f} {temp_unit}")
    st.write(f"G_min: {G_min:.2f} {flow_unit} | Relación actual G/G_min: {G/G_min:.2f}")

    if G <= G_min:
        st.error("G es insuficiente. La torre no puede operar.")
        st.stop()

    # ==================== MÉTODO DE MICKLEY (ORIGINAL RESTAURADO) ======================
    DH = (Hfin - Hini) / 20
    ...
    for _ in range(20):
        ...
        # Dibujo de segmentos del triángulo
        segmentos.append(((t_next, H_next), (t_op_next, H_next)))
        segmentos.append(((t_op_next, H_next), (t_op_next, H_star[-1])))
        segmentos.append(((t_op_next, H_star[-1]), (t_next, H_next)))

    # ==================== CÁLCULOS FINALES ====================
    t_wat_int = np.linspace(tini, tfin, 100)
    ...
    st.info(f"Altura de Relleno Z: {Z_total:.2f} {length_unit} | NtoG: {NtoG:.2f} | HtoG: {HtoG:.2f}")
    st.write(f"Agua de reposición por evaporación: {Lrep:.2f} {flow_unit}")

    # ==================== GRÁFICO (RESTAURADO CON TANGENTE) ====================
    fig, ax = plt.subplots(figsize=(10, 7))
    T_range = np.linspace(min(teq), max(teq), 200)
    ax.plot(T_range, H_star_func(T_range), 'b-', label='Línea de Equilibrio H*', linewidth=2)
    ax.plot([tini, tfin], [Hini, Hfin], 'r-', label='Línea de Operación', linewidth=2)
    ax.plot(t_air, H_air, 'ko-', label='Evolución Aire (Mickley)', markersize=3, linewidth=1)
    
    # Recta Tangente desde (tini, Hini)
    t_plot_pinch = np.linspace(tini, t_pinch_global + 5, 100)
    H_plot_pinch = Hini + m_max_global * (t_plot_pinch - tini)
    ax.plot(t_plot_pinch, H_plot_pinch, 'r--', alpha=0.5, label='Límite G_min (Tangente)')
    ax.scatter(t_pinch_global, H_pinch_global, color='red', s=80, zorder=5, label='Punto de Pinch')

    # Dibujar los segmentos de los pasos de Mickley
    ...
    ax.set_title('Diagrama de Mickley - Evolución del Aire')
    ax.legend()
    ax.grid(True)
    st.pyplot(fig)
